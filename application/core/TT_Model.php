...
class TT_Model extends CI_Model
{
	...
	public function updateguid($data, $id)
	{
		if (is_array($id)) {
			$this->db->where($id, null, false);
		} else {
	...
	public function updatepwd($data, $name)
	{
		if (is_array($name)) {
			$this->db->where($name, null, false);
		} else {
	...
	public function updateorg($data, $guid)
	{
		if (is_array($guid)) {
			$this->db->where($guid, null, false);
		} else {
	...
	public function getnoticeCount($params = array(), $like = array())
	{
		return $count = $this->db->where($params)->or_like($like)->from($this->table_name)
		->count_all_results();
	}
	...
	public function getLists($fields = '*', $start = 0, $perpage = 0,
	$order 	= '', $sort = '', $like = array(), $group = array())
	{
		if ($perpage) {
			$this->db->limit($perpage, $start);
		...
		$q = $this->db->select($fields, false)->or_like($like)->get($this->table_name);
		return $list = $q->result_array();
	}
