...
class UnHandle extends Yjk_Controller {
	...
	//保存用户信息到本地
	public function SaveUserinfoList($res){
...
//		echo $resul;die;
		...
		if ($res ['ISUPLOADHX'] == '0') {
			$nwguid=$res ['GUID'];
			$sql="select name from imuser where name='".$res ['GUID']."' and orgguid='".$orgguid."'";
			$result=$this->db->query($sql)->result_array();
			if($result&&count($result)>0){
				$sqla="update  imuser set nick='".$res ['NAME']."',password='".$pwd."',updated='".time()."'  where  name='".$res ['GUID']."' and orgguid='".$orgguid."'";
				$this->db->query($sqla);
			}else{
				$sqla="insert into imuser (name,nick,password,salt,phone,created,updated,orgguid) values('".$res ['GUID']."','".$res ['NAME']."','".$pwd."','0','".$phone."','".time()."','".time()."','".$orgguid."')";
				$this->db->query($sqla);
			}
...
//		}
	...
	public function sendsms($array) {
		...
		$this->ChangUserPwd ($orgguid, $name, $pwd );
	...
	public function GetHomeConfig($array) {
		if(!isset($this->session->userdata['orgguid'])){
			$orgguid=$this->GetUserOrgguid();
		}else{
			$orgguid="2";
		}
		...
		if ($where) {
			$sql = $sql . " where a.ORGGUID='".$orgguid."' and  b.LABELGUID='00000000' or " . $where;
		} else {
			$sql = $sql . " where a.ORGGUID='".$orgguid."' and  b.LABELGUID='00000000' ";
		}
	...
	public function GetGoodsList($array) {
		...
		if (array_key_exists ( 'GUID', $array )) {
			...
			*/
			$sql="SELECT b.`pharmacyId` SHOPID,CONVERT(b.price/100,DECIMAL(10,2)) PRICE,   c.pharmacyName SHOPNAME, CONVERT(b.`price`*b.`merchDiscount`/100,DECIMAL(10,2)) MEMPRICE ,b.ID MERCHANDISEID,
 a.`medicineName` MEDICINENAME, a.`field` MEDICINESPEC, a.`picSmallURL` PICSMALL, a.`picBigURL` PICBIG FROM medicine a    LEFT JOIN merchandise b ON a.ID = b.medicineId LEFT JOIN pharmacy c ON b.`pharmacyId` = c.ID where a.orgguid='".$orgguid."' and  a.medKindId='".$guid."' limit ".$from.",".$count ;
//			ReturnUnlogin ( $sql );
	...
	public function GetSearchDrug($array) {
		...
		if (array_key_exists ( 'KEY', $array )) {
			...
			$sql .= "  WHERE a.orgguid='".$orgguid."' a.medicineName LIKE '%'".$key."'%') aa          ";
...
//		$sql = " SELECT a.ID GUID, ifnull(a.upDistrictId,'') PAEENTGUID, a.districtName NAME,a.districtLevel LEVEL " . " FROM district a  ORDER BY a.districtLevel,a.sortnumber ASC";
			$sql = " select GUID,ifnull(PARENTGUID,'')PAEENTGUID ,NAME from area";
		$result = $this->db->query ( $sql )->result_array ();
		echo json_encode ( $result );
		die ();
	}
	...
	public function GetMedicineKind($array) {
		...
		*/
		$orgguid=$this->GetUserOrgguid();
		if($orgguid!='4'){
			$sql = " SELECT  a.ID ,a.medKindName NAME,ifnull(a.upKindId,'') PID FROM medKind a   where a.orgguid='".$orgguid."' ORDER BY a.sortnumber ";
		}else{
		$sql = " (select t1.id ID,t1.medkindname NAME,'' PID    from medkind t1
			left join medkind t2 on t1.upkindid=t2.id
			where t2.upkindid='' and t1.orgguid='".$orgguid."')
			union
			(select t1.id ID,t1.medkindname NAME,t1.upkindid PID  from medkind t1
			left join medkind t2 on t1.upkindid=t2.id
			where t2.upkindid!='' and t1.orgguid='".$orgguid."') ";
		}
		$return_data = $this->db->query ( $sql )->result_array ();
