...
class Handle extends Yjk_Controller {
	...
	public function GetUserAddressList($array) {
		...
		if (array_key_exists ( 'USERGUID', $array )) {
			$sql = " SELECT a.ID ADDRESSID,a.memberId USERGUID,a.receiver RECEIVER,a.contactPhone CONTACTPHONE,a.district DISTRCTID," . " a.address ADDRESS,a.isDefault ISDEFAULT,a.addressType ADDRESSTYPE FROM recaddress a WHERE a.memberId = '" . $array ['USERGUID'] . "' ";
	...
	public function Changepwd($array) {
		...
		$resulta = $this->ChangUserPwd ( $username, $newpld );
		if ($resulta) {
			echo json_encode ( '' );
		} else {
	...
	public function InsertIntoCar($array) {
		...
		if (array_key_exists ( 'USERGUID', $array )&&array_key_exists ( 'GOODSGUID', $array )) {
			$user_guid = $array ['USERGUID'];// 用户guid
...
// 			die ();
			...
			for($i = 0; $i < count ( $goods_list ); $i ++) { // 循环商品
				...
				if (! in_array ( $shop_id, $shop_list )) { // 数组中不存在药店ID，则需要新生成一个订单
					...
					$order_info = array (
							...
							'orderno' => date ( "YmdHis", time () ) . rand ( 100, 999 ),
	...
	public function getMyOrders($array) {
		...
		if (array_key_exists ( "USERGUID", $array )) {
			...
			$order_result = $this->db->query ( $sql )->result_array ();
	...
	public function GetUserAssessment($array) {
		...
		if (array_key_exists ( "MERCHANDISEID", $array ) && array_key_exists ( "USERGUID", $array )) {
			...
			$from = $page * $count; // 计算起始查询条数
			$sql = " SELECT FROM_UNIXTIME(CREATEDATE,'%Y-%m-%d %H:%I:%S') CREATEDATE,ISAPPEND,ASSESSTXT,QUALITYLEVEL,";
